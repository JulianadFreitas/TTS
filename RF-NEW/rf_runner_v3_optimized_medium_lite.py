...
def get_param_grid_medium_lite() -> Dict[str, Any]:
    """
    Grid MÉDIO-LITE - balanceado entre quick e medium.
    ~96 combinações = 480 modelos (5-fold CV)
    Tempo estimado: 30-45 minutos
    """
    return {
        "clf__strategy": ["none", "oversample"],  # 2 (remove undersample)
        "clf__n_estimators": [200, 500],  # 2
        "clf__max_depth": [None, 20, 40],  # 3
        "clf__max_features": ["sqrt", 0.5],  # 2
        "clf__min_samples_split": [2, 10],  # 2
        "clf__min_samples_leaf": [1, 2, 4],  # 3
    }
...
def get_param_grid_medium() -> Dict[str, Any]:
    """
    Grid MÉDIO - busca mais abrangente (REDUZIDO).
    ~108 combinações = 540 modelos (5-fold CV)
    Tempo estimado: 45min-1h
    """
    return {
        "clf__strategy": ["none", "oversample"],  # 2 (era 3, removido undersample)
        "clf__n_estimators": [200, 500],  # 2
        "clf__max_depth": [None, 20, 40],  # 3
        "clf__max_features": ["sqrt", 0.5],  # 2
        "clf__min_samples_split": [2, 10, 20],  # 3 (mantido)
        "clf__min_samples_leaf": [1, 2, 4],  # 3
    }
    # Total: 2 * 2 * 3 * 2 * 3 * 3 = 216 combinações (era 162 mas mais focado)
...
def evaluate_rf_pre_post(
    ...
    search_method:
        ...
        - "medium-lite": Grid balanceado, ~144 combinações (~30-45 min) ⭐ NOVO
        - "medium": Grid médio, ~216 combinações (~45min-1h)
...
if __name__ == "__main__":
    ...
    # OPÇÃO 3 - BALANCEADO (NOVO!): 30-45 minutos, 144 combinações
    # SEARCH_METHOD = "medium-lite"
    
    # OPÇÃO 4 - MÉDIO (REDUZIDO): 45min-1h, 216 combinações

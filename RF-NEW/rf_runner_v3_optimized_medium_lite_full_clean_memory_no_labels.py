...
def select_features(
    ...
    """Select features based on mode (pre or post) WITHOUT label one-hot features."""
    ...
    if mode == "pre":
        ...
        wanted = pre_cols + post_only_cols
    else:
        raise ValueError("mode must be 'pre' or 'post'")

    # Guarantee we drop any label one-hot columns if they appear in pre_cols by accident
    wanted = [
        c for c in wanted
        if c in df.columns and not (c.startswith(LABEL_PREFIX) and c != "CONT_label_count")
    ]

    used = list(dict.fromkeys(wanted))
    if not used:
        raise ValueError(f"No features selected for mode='{mode}'. Check your column lists.")
...
def evaluate_rf_pre_post(
    ...
    test_size: float = 0.2,
    cv_splits: int = 5,
    search_method: str = "medium-lite",  # "quick" | "medium" | "full" | "random"
...
if __name__ == "__main__":
    ...
    evaluate_rf_pre_post(
        ...
        results_root="outputs_rf_v3_no_label_features",
        random_state=42,
        test_size=0.2,
        cv_splits=5,
        search_method=SEARCH_METHOD,
